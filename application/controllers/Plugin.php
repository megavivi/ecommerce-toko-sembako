...
class Plugin extends CI_Controller {
	...
	{
		$data['title'] = 'Ubah Data Ongkir';
		$data['plugin'] = $data['ongkir'] = true;
		$data['provinsi'] = $this->Wilayah_m->provinsi();
		$data['datakurir'] = $this->Wilayah_m->kurir();
		// $data['dataservice'] = $this->Wilayah_m->service();
		$data['allongkir'] = $this->Plugin_m->allOngkir();
		$data['o'] = $this->db->get_where('ongkir',['idongkir'=>$id])->row();
		$data['content'] = 'backend/ongkir_edit';
		$this->load->view('backend/index', $data);
	}

	public function updateOngkir($id)
	{
		$this->db->where('idongkir',$id)->update('ongkir', $this->dataOngkir());
		$this->toastr->success('Update Successfully');
		redirect('plugin/ongkir');
		// var_dump($this->data());
	...
	{
		$this->db->where('idkurir',$id)->update('kurir', $this->dataService());
		$this->toastr->success('Update Successfully');
		redirect('plugin/kurir');
		// var_dump($this->data());
	...
	*/
	...
	public function viewOngkir()
	{
		$id = $this->input->post('id', true);
		$data = $this->db->get_where('ongkir',['idongkir'=>$id])->row();
		echo json_encode($data);
	}
	/**
	* Delete By ID
	...
	*/
	...
	public function deleteOngkir()
	{
		...
		$id = $this->input->post('idx');
		for ($i=0; $i < count($id); $i++) { 
			$this->Plugin_m->delete_permanenOngkir($id[$i]);
		}
	}
	/**
	* Restore By ID
