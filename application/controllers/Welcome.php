...
class Welcome extends CI_Controller {
	...
	public function __construct()
	{
		...
		$this->load->config('midtrans'); // Load konfigurasi Midtrans
        Config::$serverKey = $this->config->item('server_key');
        Config::$isProduction = $this->config->item('is_production') ? true : false;
	...
	public function process_payment($idorder) {
        // Proses pembayaran

		$query = $this->db->select(['pesanan.total_harga','pesanan.code','users.user_fullname','users.user_telp'])
		->join('users','users.idusers = pesanan.user_id','left')
		->where('pesanan.idorder',$idorder)
		->get('pesanan')->row();
        $params = array(
			'transaction_details' => array(
				'order_id' => $query->code,
				'gross_amount' => $query->total_harga,
			),
			'customer_details' => array(
				'first_name' => $query->user_fullname,
				'phone' => $query->user_telp,
			),
		);
		...
		if ($transaction == 'capture') {
			if($fraud == 'accept'){
				// TODO set payment status in merchant's database to 'Success'
					$pesanan = $this->db->get_where('pesanan',['code'=>$order_id])->row();
					$query = $this->db->where('order_id',$pesanan->idorder)->update('pembayaran',[
						'total'=>$notif->gross_amount,
						'status'=>'verified',
						'keterangan'=>'Pembayaran Via Midtrans',
						'tgl_bayar'=>strtotime($notif->transaction_time)
					]);
					$qpesanan = $this->db->where('code',$order_id)->update('pesanan',[
						'status_bayar'=>'lunas',
