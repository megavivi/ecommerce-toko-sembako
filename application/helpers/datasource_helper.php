...
if (!function_exists('bayar')){
	function bayar($id){
		$CI =& get_instance();
		$CI->db->select(['pesanan.code', 'FROM_UNIXTIME(pesanan.datetime) as tanggal', 'pesanan.total_harga', 'pembayaran.*','FROM_UNIXTIME(pembayaran.tgl_bayar) as tglbayar'])
		->join('pesanan','pesanan.idorder = pembayaran.order_id','left')
		->order_by('pembayaran.idpembayaran', 'desc');
		return $CI->db->get_where('pembayaran',['pesanan.user_id'=>$id])->result();
	}
}
/**
* Get Detail Order
