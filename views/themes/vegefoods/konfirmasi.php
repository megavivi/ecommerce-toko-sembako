...
<script>
        function pay(id) {
            fetch('<?php echo site_url("welcome/process_payment/"); ?>/'+id)
                .then(response => response.json())
                .then(data => {
                    snap.pay(data, {
                        onSuccess: function(result) {
                            console.log('Payment successful!', result);
                            window.location.href = "<?php echo site_url('public/konfirmasi'); ?>";
                        },
        ...
        <div class="row block-9">
            ...
            <div class="col-md-6 d-flex mb-3" id="form-addbayar">
                <form action="<?=base_url('user/addKonfirmasi');?>" method="post" class="bg-white p-5 contact-form"
                    ...
                    <hr><button id="bayar" type="button" class="btn btn-info mt-2">SUDAH DIBAYAR</button>
                    <?php
                        } else {
                            echo '<hr><button id="bayar" type="button" class="btn btn-success mt-2" onclick="pay('.$b->order_id.')">Bayar Tagihan</button>';
                        }
                    ?>
